...
> 成片：[`film/skills-hub-promo.mp4`](film/skills-hub-promo.mp4)（1920×1080 · 60fps · H.264 + AAC）
...
实时预览：`npx http-server . -c-1` 后打开 `/src/index.html?play`（点一下画面开始，跟随音频播放）；`?t=21.5` 定格到某一秒。
...
promo/
  src/
    ...
    index.html           舞台
    styles/tokens.css    ★ 设计令牌（新前端的视觉语言）
...
|---|---|---|
| 基底 | 浅色、白底灰阶 | 深色四档表面 `#050607 → #1c2228` |
| 品牌色 | 控件与导航禁止品牌色 | 一支信号色 Volt `#c8f53c`，只给「当前 / 主动作 / 已链接」 |
...
**建议**：信息架构、组件清单、动效口径与现行规范一致，可以直接复用；冲突点在「深色 + 品牌信号色」这两条，它们正是 `ui-design-v1` 明文禁止的。按 AGENTS.md「遇到架构矛盾先修文档」，迁回的顺序应当是：

1. 先提一版 `ui-design-v2`（或 v1 修订），写明为什么要引入深色基底和唯一信号色，以及信号色的使用边界；
