...
const secondaryBtn =
  "rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink-strong hover:border-line-strong disabled:text-ink-faint";
const dangerBtn =
  "rounded-lg border border-red-200 bg-white px-3 py-2 text-sm text-red-700 hover:border-red-300 disabled:border-line disabled:text-ink-faint";
...
export function ReportPanel(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    void fetchVerify()
      ...
      .catch((e: unknown) => {
        if (!cancelled) setLoadError((prev) => prev ?? fail(e));
      });
    void fetchDoctor()
      .then((d) => {
    ...
    void fetchBackups()
      .then((b) => {
        if (cancelled) return;
        setBackups(b);
        setSnapshotId(b.latest ?? b.snapshots[0]?.snapshotId ?? "");
      })
      .catch((e: unknown) => {
        if (!cancelled) setLoadError((prev) => prev ?? fail(e));
      });
  ...
  return (
    <div className="space-y-6">
      <section data-testid="reset-panel" className="rounded-xl border border-line p-4">
        <h3 className="text-sm font-medium text-ink-strong">一键恢复到初始化前</h3>
        <p className="mt-1 text-xs text-ink-faint">
          按备份快照还原各客户端 skills，旁路旧库存后再自动收录。先预览，再输入 reset 确认。
        </p>
        {backups === null && loadError === null && <p className="mt-3 text-sm text-ink-mid">正在读取备份快照…</p>}
        {backups === null && loadError !== null && <p className="mt-3 text-sm text-red-700">{loadError}</p>}
        {backups !== null && backups.snapshots.length === 0 && (
          <p className="mt-3 text-sm text-ink-mid">尚无备份快照，无法恢复。</p>
        )}
        {backups !== null && backups.snapshots.length > 0 && (
          <div className="mt-3 space-y-3">
            <label className="block text-xs text-ink-mid">
              快照
              <select
                className="mt-1 block w-full rounded-lg border border-line bg-white px-2 py-1 text-sm text-ink-strong"
                value={snapshotId}
                onChange={(e) => {
      ...
      <section>
        ...
        {verify === null ? (
          <p className="mt-2 text-sm text-ink-mid">正在校验…</p>
        ) : (
      ...
      <section>
        ...
        {doctor === null ? (
          <p className="mt-2 text-sm text-ink-mid">正在自检…</p>
        ) : (
          <ul className="mt-2 space-y-1">
            <Status
