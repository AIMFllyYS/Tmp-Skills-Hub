/**
 * Agent 页(app-shell-v2.md §Agent):页头模型选择 + 消息区流式渲染 + 输入区。
 * 完全自治:自己拉模型白名单、自己管会话,不接收 App 的数据 props。
 */
...
export function AgentPage(): React.JSX.Element {
  const { entries, busy, model, setModel, applyModelDefault, send, stop, reset } = useAgentChat();
  ...
  useEffect(() => {
    ...
    void fetchAgentModels()
      .then((res) => {
        ...
        // 无已存模型或已存模型不在白名单时回落默认
        applyModelDefault(res.defaultModel, res.models.map((m) => m.id));
      })
      .catch((e) => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
        ...
        {entries.map((entry, i) =>
          entry.kind === "tool" ? (
            <ToolCallCard key={"tool-" + entry.callId + "-" + String(i)} entry={entry} />
          ) : (
            <MessageBubble key={String(i)} entry={entry} />
          ),
        )}
      </div>
