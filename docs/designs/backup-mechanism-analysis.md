...
> 状态：分析稿。本文只回答「怎么备份」，不含代码实现；落地时按本文拆 issue。
> 日期：2026-08-16
...
- 不含：跟随 symlink/junction 递归（防环）。链接本身记录进清单（类型 + 目标），不复制链接目标。
...
<storeRoot>/
  backups/
    <快照ID>/          # 如 2026-08-16T14-30-00Z
      manifest.json    # 清单:每份副本 {root, 相对路径, hash, size, mtime, link?}
      blobs/           # 内容寻址库:<sha256> 一个文件一份内容
...
- 跳过 symlink/junction 跟随，清单记录链接本身（防环、防备份风暴）。
